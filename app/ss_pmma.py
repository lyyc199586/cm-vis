...
for s in [drucker]:
    data_dir = f"../data/strength/ss_{s['mname']}_{s['stype']}_props{s['props']}_srange{s['srange']}.npy"
    plotter = SurfacePlotter(data_dir)
    plotter.plot(dim=3, ax=ax, label=labels[s["stype"]])
...
# %% save plot
save_dir = "../out/strength/"
ax.figure.savefig(save_dir + "ss_pmma_3d_drucker.png")
# %%
